################################################################################
############# Analiza udarnog testa na oplati 2MVA 19.09.2020. ###############
################################################################################
...
# Mjerenja su provedena na oplati. Odabrana je VN strana
# Na oplati je raspoređeno ukupno 5 mjernih točaka. Čekićem se udaralo na četiri
# mjesta u blizini, a vibracije su se mjerile u istoj točki. Mjerenja su
# korištena kao replikacije.
# Dodatno je u svakoj točki jednom mjeren faktor prigušenja, ali na način da
# transformator radi i da se naglo ugasi. Usporedba rezultata.
...
# input.df %>%
#   rename(Tocka=point.id, x=loc.x, y=loc.y) %>%
#   write_excel_csv2('2MVA/DoE/Oplata_i_prigusenje/impact_stvarne_koordinate.csv')
...
#       impact.path <- paste0(
#         '2MVA/mjerenja/impact/', 'p', input.df$point.id[i], '_', j
#           )
#       print(impact.path)
#       dat <- dat %>%
#         add_row(
...
#   replication = 1,
#   decimal.separator = ',',
#   file.loc = paste0('2MVA/mjerenja/impact/gasenje_p', input.df$point.id[1])
# )

# Data input gasenje_p2
# dat <- dat %>%
#   add_row(
#     !!!VibData(
#       point.id = input.df$point.id[2],
#       loc.x = input.df$loc.x[2],
#       loc.y = input.df$loc.y[2],
#       rib = F,
#       replication = 1,
#       decimal.separator = ',',
#       file.loc = paste0('2MVA/mjerenja/impact/gasenje_p', input.df$point.id[2])
# ))

# Data input gasenje_p3
# dat <- dat %>%
#   add_row(
#     !!!VibData(
#       point.id = input.df$point.id[3],
#       loc.x = input.df$loc.x[3],
#       loc.y = input.df$loc.y[3],
#       rib = F,
#       replication = 1,
#       decimal.separator = ',',
#       file.loc = paste0('2MVA/mjerenja/impact/gasenje_p', input.df$point.id[3])
# ))

# Data input gasenje_p4
# dat <- dat %>%
#   add_row(
#     !!!VibData(
#       point.id = input.df$point.id[4],
#       loc.x = input.df$loc.x[4],
#       loc.y = input.df$loc.y[4],
#       rib = F,
#       replication = 1,
#       decimal.separator = ',',
#       file.loc = paste0('2MVA/mjerenja/impact/gasenje_p', input.df$point.id[4])
# ))

# Data input gasenje_p5
# dat <- dat %>%
#   add_row(
#     !!!VibData(
#       point.id = input.df$point.id[5],
#       loc.x = input.df$loc.x[5],
#       loc.y = input.df$loc.y[5],
#       rib = F,
#       replication = 1,
#       decimal.separator = ',',
#       file.loc = paste0('2MVA/mjerenja/impact/gasenje_p', input.df$point.id[5])
# ))
...
# Rename rib to off
# dat <- dat %>% rename(off = rib)

# Check sample length
# dat %>% group_by(off, point.id, replication) %>%
#   mutate(d.freq.T = sample.rate/length(na.omit(velocity.amp))/2)  %>%
#   summarise(d.freq = unique(d.freq),
#             d.freq.T = unique(d.freq.T),
#             sample.num = unique(sample.num),
#             sample.num.T = 2*length(na.omit(velocity.amp))) %>% View()
  
# Recalculate FFT resolution because of sample length change
# dat <- dat %>% group_by(off, point.id, replication) %>%
#   mutate(d.freq = sample.rate/length(na.omit(velocity.amp))/2) %>% ungroup()

# Sanity data check
# dat %>% glimpse()
# dat %>% select(off, replication, point.id, range, d.time, d.freq, sample.rate, sample.num) %>% distinct() %>% View()
# dat %>% group_by(off, point.id, replication) %>% summarise(
...
# dat %>%
#   select(off, point.id, replication, peak.freq.orig, peak.vel.orig, peak.frequency, peak.velocity.amp) %>%
...
# Check subtitles
# dat %>% group_by(off, point.id, replication) %>% select(subtitle) %>% distinct() %>% View()

# Save current workspace
# save.image('2MVA/impact.RData')

# Load workspace
load('2MVA/impact.RData')
...
dat %>% filter(off == T) %>%
  filter(time >= 0.2 & time <= 2) %>%
  select(time, velocity, point.id, off, subtitle, loc.x, loc.y, replication) %>%
  mutate(
    point.id = paste0('pt', point.id, ' (', loc.x, ', ', loc.y, ')'),
  ...
  ggtitle("Udarni test na VN strani - Vremenska domena") +
  theme_bw() +
  theme(
...
ggsave(filename = '2MVA/preliminarna_obrada/impact/slike/udarni_brzina_vrijeme.png',
       width = 25, height = 17, units = 'cm', dpi = 320)

# Gasenje transformatora
# Vremenska domena plot
dat %>% filter(off == F) %>%
  filter(time >= 0.5 & time <= 3.3) %>%
  select(time, velocity, point.id, off, subtitle, loc.x, loc.y, replication) %>%
  mutate(
    point.id = paste0('pt', point.id, ' (', loc.x, ', ', loc.y, ')'),
    replication.txt = paste0('rep: ', replication)
  ...
  ggplot(aes(x = time, y = velocity)) +
  facet_wrap(factor(point.id, levels = mixedsort(unique(point.id))) ~ .,
             ncol = 5) +
  geom_line(aes(color = point.id), size = 0.4) +
  scale_y_continuous(
    name = expression('Brzina ['*mu*'m/s]'),
  ...
  ggtitle("Gašenje transformatora na VN strani - Vremenska domena") +
  theme_bw() +
  theme(
    axis.line = element_line(size=0.5, colour = "black"),
    plot.title = element_text(hjust = 0.5),
...
ggsave(filename = '2MVA/preliminarna_obrada/impact/slike/gasenje_brzina_vrijeme.png',
       width = 25, height = 6, units = 'cm', dpi = 320)
################################################################################
################################################################################
# Transformator ugašen - udarni
# Frekvencijska domena plot
dat %>% filter(off == T) %>%
  filter(frequency >= 90 & frequency <= 110) %>%
  select(frequency, velocity.amp, point.id, off, subtitle) %>%
  drop_na() %>%
  mutate(
...
dat <- dat %>%
  group_by(off, point.id, replication) %>%
...
################################################################################
################################################################################
# Calculate damping estimate - udarni
npeaks <- 300
cutoff.time.min <- 0.2
cutoff.time.max <- 2.0
dat_fit <- dat %>%
  ...
  nest_by(off, point.id, replication) %>%
  mutate(
...
# Damping plot - ugasen
dat_fit %>% select(off, point.id, replication, data, fit.res) %>%
  filter(off == T) %>%
  # filter(point.id == 5 & replication == 4) %>%
  unnest(cols = c('data', 'fit.res')) %>%
  select(
    off, point.id, replication, loc.x, loc.y, time, time.fit, vel.filt, peak.vel,
  ...
  scale_y_log10(
    name = expression('Brzina ['*mu*'m/s]'),
    breaks = c(1e3, 1e2, 1e1, 1e-1, 1e-3),
  ...
  ggtitle("Prigušenje na 100 Hz - udarni test") +
  theme_bw() +
  theme(
    axis.line = element_line(size=0.5, colour = "black"),
    plot.title = element_text(hjust = 0.5),
...
ggsave(filename = '2MVA/preliminarna_obrada/impact/slike/udarni_damp_100_hz.png',
       width = 25, height = 17, units = 'cm', dpi = 320)

# Summary faktora prigusenja po replikacijama - udarni
dat_fit_rep <- dat_fit %>% select(-fit.log, fit.res) %>%
  unnest(cols = c('data', 'log.dec', 'adj.r.squared', 'rss')) %>%
  select(off, point.id, replication, peak.vel, damp, log.dec, adj.r.squared, rss) %>%
  drop_na() %>%
  summarise(
    .groups = 'drop_last',
    rep.damp = mean(damp),
...
dat_fit_rep %>%
  ...
  rename(Tocka = point.id, Replikacija = replication, Test = off,
         'Broj peakova' = num.obs.rep) %>%
  write_excel_csv2(path = '2MVA/preliminarna_obrada/impact/udarni_damp_replication_summary.csv')
  
# Summary faktora prigusenja po tockama - udarni
...
dat_fit_pt %>%
  ...
  rename(Tocka = point.id, Test = off, 'Broj replikacija' = num.obs.point) %>%
  write_excel_csv2(path = '2MVA/preliminarna_obrada/impact/udarni_damp_point_summary.csv')

# Summary faktora prigusenja po testovima - udarni
...
dat_fit_glob %>%
  ...
  rename(Test = off, 'Broj tocaka' = num.obs.glob) %>%
  write_excel_csv2(path = '2MVA/preliminarna_obrada/impact/udarni_damp_glob_summary.csv')

################################################################################
################################################################################
# Calculate damping estimate - gasenje
npeaks <- 300
cutoff.time.min <- 1.7
cutoff.time.max <- 3.2
dat_fit <- dat %>%
...
dat_fit %>% select(off, point.id, replication, data, fit.res) %>%
  ...
  ggplot(aes(x = time, y = vel.filt)) +
  facet_wrap(
    factor(point.id, levels = mixedsort(unique(point.id))) ~ .,
    ncol = 5
    ) +
  geom_line(aes(color = point.id), size = 0.4) +
  geom_point(aes(y = peak.vel, x = time.fit, fill = 'Vršne vrijednosti'),
  ...
  ggtitle("Prigušenje na 100 Hz - test gašenjem transformatora") +
  theme_bw() +
  theme(
    axis.line = element_line(size=0.5, colour = "black"),
    plot.title = element_text(hjust = 0.5),
...
ggsave(filename = '2MVA/preliminarna_obrada/impact/slike/gasenje_damp_100_hz.png',
       width = 25, height = 8, units = 'cm', dpi = 320)

# Summary faktora prigusenja po replikacijama - gasenje
dat_fit_rep <- dat_fit %>% select(-fit.log, fit.res) %>%
  unnest(cols = c('data', 'log.dec', 'adj.r.squared', 'rss')) %>%
  select(off, point.id, replication, peak.vel, damp, log.dec, adj.r.squared, rss) %>%
  drop_na() %>%
...
dat_fit_rep %>%
  ...
  rename(Tocka = point.id, Replikacija = replication, Test = off,
         'Broj peakova' = num.obs.rep) %>%
  write_excel_csv2(path = '2MVA/preliminarna_obrada/impact/gasenje_damp_replication_summary.csv')
  
# Summary faktora prigusenja po tockama - gasenje
dat_fit_pt <- dat_fit_rep %>%
  summarise(
    .groups = 'drop_last',
    point.damp = weighted.mean(rep.damp, w = num.obs.rep),
...
dat_fit_pt %>%
  ...
  rename(Tocka = point.id, Test = off, 'Broj replikacija' = num.obs.point) %>%
  write_excel_csv2(path = '2MVA/preliminarna_obrada/impact/gasenje_damp_point_summary.csv')

# Summary faktora prigusenja po testovima - gasenje
dat_fit_glob <- dat_fit_pt %>%
  summarise(
    .groups = 'drop_last',
    glob.damp = weighted.mean(point.damp, w = num.obs.point),
...
dat_fit_glob %>%
  ...
  rename(Test = off, 'Broj tocaka' = num.obs.glob) %>%
  write_excel_csv2(path = '2MVA/preliminarna_obrada/impact/gasenje_damp_glob_summary.csv')
