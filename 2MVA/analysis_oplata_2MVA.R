################################################################################
################# Analiza mjerenja na oplati 2MVA 19.09.2020. ##################
################################################################################
...
# Mjerenja su provedena na 4 strancice kotla pravokutnog oblika.
# Mjerene su VN strana, NN strana , bok lijevo BL i bok desno BD (ukupno 4
# povrsina).
# Dodatni elemenata na oplati nije bilo.
# Ishodište mjernog koordinatnog sustava # se nalazi u donjem lijevom kutu.
# Na bočnim mjernim povrsinama raspoređeno je 15 mjernih točaka.
...
#   write_excel_csv2('2MVA/DoE/Oplata_i_prigusenje/oplata_stvarne_koordinate.csv')
...
#   oplata.path <- '2MVA/mjerenja/oplata/'
...
#   replication = 1,
#   decimal.separator = ',',
#   file.loc = paste0('2MVA/mjerenja/oplata/vn_1')
# )
...
# Input the rest of data points for VN, NN, BL and BD
...
# save.image('2MVA/oplata.RData')

# Load workspace
load('2MVA/oplata.RData')
...
dat %>% filter(
  ...
  facet_wrap(factor(subtitle,
                    levels = mixedsort(unique(subtitle), decreasing = T)) ~ .,
             ncol = 3) +
...
ggsave(filename = '2MVA/preliminarna_obrada/oplata/slike/brzina_vrijeme.png',
...
dat %>% filter(
  ...
  ggtitle("Frekvencijska domena - oplata") +
  theme_bw() +
  theme(
    axis.line = element_line(size=0.5, colour = "black"),
    plot.title = element_text(hjust = 0.5),
...
ggsave(filename = '2MVA/preliminarna_obrada/oplata/slike/brzina_freq.png',
       width = 17, height = 15, units = 'cm', dpi = 320)
...
dat_summary %>%
  write_excel_csv2(
    path = '2MVA/preliminarna_obrada/oplata/oplata_summary.csv'
    )
...
ggsave(
  filename = '2MVA/preliminarna_obrada/oplata/slike/amplitude_100_hz_oplata_vn_nn.png',
...
ggsave(
  filename = '2MVA/preliminarna_obrada/oplata/slike/amplitude_100_hz_oplata_bl_bd.png',
       width = 17, height = 15, units = 'cm', dpi = 320)
...
# Determining cutoff
dat_surf %>% filter(side == 'VN') %>% select(x, y) %>%
  summarise(
    max.x = max(x),
    min.x = min(x),
    max.y = max(y),
...
dat_disp_vel <- dat_surf %>%
  select(-amp.x.100, -adj.r.sq, -x.VN) %>%
...
dat_disp_disp <- dat_surf %>%
  select(-amp.v.100, -adj.r.sq, -x.VN) %>%
  mutate(point.id = as.character(point.id)) %>%
...
ggsave(filename = '2MVA/preliminarna_obrada/oplata/slike/contour_disp_vn_nn_100_hz.png',
       width = 25, height = 11, units = 'cm', dpi = 320)
...
ggsave(filename = '2MVA/preliminarna_obrada/oplata/slike/contour_disp_bl_bd_100_hz.png',
       width = 17, height = 15, units = 'cm', dpi = 320)
...
ggsave(filename = '2MVA/preliminarna_obrada/oplata/slike/contour_vel_vn_nn_100_hz.png',
       width = 25, height = 11, units = 'cm', dpi = 320)
...
ggsave(filename = '2MVA/preliminarna_obrada/oplata/slike/contour_vel_bl_bd_100_hz.png',
       width = 17, height = 15, units = 'cm', dpi = 320)
